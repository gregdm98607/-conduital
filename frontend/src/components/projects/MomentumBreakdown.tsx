...
function FactorBar({ factor }: { factor: MomentumFactor }) {
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      ...
      <span className="w-28 text-right text-xs text-gray-500 dark:text-gray-400 shrink-0">{factor.detail}</span>
    </div>
  );
}

export function MomentumBreakdown({ projectId }: MomentumBreakdownProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const { data: breakdown, isLoading } = useQuery({
    queryKey: ['momentum-breakdown', projectId],
    queryFn: () => api.getMomentumBreakdown(projectId),
    enabled: isExpanded,
    staleTime: 60_000,
  });
  ...
  return (
    <div>
      ...
      <button
        ...
        title="Show momentum score breakdown"
      >
