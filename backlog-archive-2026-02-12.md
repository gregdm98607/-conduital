...
| ID | Description | Status | Notes |
|----|-------------|--------|-------|
| ROADMAP-007 | **GTD Weekly Review with AI Advisors** | **Done** | AI weekly review co-pilot with portfolio narrative + per-project insights |
| ROADMAP-002 | **AI Features Integration** | **Done** | Proactive analysis, task decomposition, rebalancing, energy recs |
...
| DEBT-107 | New AI API methods missing `signal?: AbortSignal` parameter | `api.ts:875-881` | **Done** (Session 5) |
| DEBT-109 | `projectInsight.mutate()` can fire multiple times for same project — no deduplication | `WeeklyReviewPage.tsx:95` | **Done** (Session 5) |
| DEBT-110 | `completeReview.mutate()` has no `onError` callback — silent failure | `WeeklyReviewPage.tsx:365` | **Done** (Session 4 — already fixed) |
| DEBT-111 | N+1 query in `get_weekly_review_ai_summary` — per-project MomentumSnapshot loop | `intelligence.py:922-940` | **Done** (Session 5) |
| DEBT-112 | JSON fence stripping in AI service uses naive string ops — fragile | `ai_service.py:482-502` | Open |
| DEBT-113 | `getWeekKey()` ISO week calculation doesn't match ISO 8601 Thursday rule | `WeeklyReviewPage.tsx:27-30` | **Done** (Session 5) |
...
| DEBT-114 | `AIReviewSummary` renders `attention_items` without null check — crash risk | `AIReviewSummary.tsx:104-107` | **Done** (Session 5) |
...
| ~~BACKLOG-101~~ | ~~Dashboard Stats Block Visual Consistency~~ | ~~Transform Dashboard stats cards to match Today's Focus accent-bar style.~~ **Done** (Session 5) |
...
| ~~BUG-028~~ | ~~**AI Insights on Project Detail 500 Error**~~ | ~~ProjectDetail AI insight cards return "Requests failed with status code 500".~~ **Fixed** (Session 5) — Sanitized backend error messages, improved frontend Axios error detection |
| ~~BUG-029~~ | ~~**Proactive Analysis Dashboard Failure**~~ | ~~Proactive Analysis on Dashboard fails with "Analysis failed for this project."~~ **Fixed** (Session 5) — Pass exception type name in error field, improved frontend error status detection |
| ~~BACKLOG-140~~ | ~~**Dashboard: Move Top Next Actions Above Areas**~~ | ~~Reorder Dashboard sections so Top Next Actions appears before Areas widget.~~ **Done** (Session 5) |
| BACKLOG-141 | **List View Column Header Sorting** | When standardizing list views (see BACKLOG-076), add sort-by-column-header functionality. Clickable headers with ascending/descending toggle. |
| BACKLOG-142 | **localStorage Key Namespacing** | All localStorage keys (`weeklyReviewChecklist`, `pt-settings-sections`, etc.) need user/session namespace prefix to prevent conflicts in multi-user browser scenarios. |
...
| BUG-027 | Rebalance due_date type check (isinstance datetime → date arithmetic) | 2026-02-12 |
| DEBT-094 | handleCreateAll fires N parallel mutations (async/await loop fix) | 2026-02-12 |
| DEBT-095 | Missing error state in AIEnergyRecommendations + AIRebalanceSuggestions | 2026-02-12 |
| DEBT-103 | Proactive analysis leaks raw exception strings (sanitized error message) | 2026-02-12 |
| ROADMAP-002 | AI Features Integration — 4 endpoints + 4 frontend components (v1.1.0 Session 2) | 2026-02-12 |
| ROADMAP-007 | AI Weekly Review Co-Pilot — portfolio narrative, attention items, per-project insights | 2026-02-12 |
...
*Last updated: 2026-02-12 (Session 4: warmup fixes + ROADMAP-007 AI Weekly Review Co-Pilot)*
