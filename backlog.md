...
| ~~BACKLOG-103~~ | ~~Project Review Frequency + Mark Reviewed Next Date~~ ✅ Done | **Fix:** Replaced manual `next_review_date` date picker with `review_frequency` dropdown (daily/weekly/monthly) matching Areas pattern. `next_review_date` auto-calculated from frequency + last review. Migration backfills existing projects. |
...
| ~~BACKLOG-112~~ | ~~Export Preview: Refresh after download~~ ✅ Done | **Fix:** Export preview auto-refreshes after JSON or DB backup download completes. |
